...
try {
  execSync(`${npmCmd} run build`, { cwd: strapiBackendPath, stdio: 'inherit' });
...
try {
  // Start Strapi in a separate detached process
  require('child_process').exec(
    `"${npmCmd}" run develop`,
    { cwd: strapiBackendPath, detached: true, stdio: 'ignore' }
  );
  console.log('   ℹ️ Server spawned, waiting 90 seconds for initialization...');
} catch (e) {
  console.error('   ✗ Failed to start Strapi:', e.message);
  process.exit(1);
}
setTimeout(async () => {
